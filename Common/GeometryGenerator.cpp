...
void GeometryGenerator::CreateSphere(float radius, UINT sliceCount, UINT stackCount, MeshData& meshData)
{
	...
	//向上为y轴，向里为z轴的球坐标系，theta为与x轴的夹角，phi为与y轴的夹角
	float phiStep   = XM_PI/stackCount;//行
	float thetaStep = 2.0f*XM_PI/sliceCount;//列
	...
	for(UINT i = 1; i <= stackCount-1; ++i)
	{
		...
		// Vertices of ring.//每一圈实际上是sliceCount+1个点，0和360是两个不同的点，虽然位置相同但是不同的纹理位置
		for(UINT j = 0; j <= sliceCount; ++j)
		{
			...
			v.Position.x = radius*sinf(phi)*cosf(theta);//sin求导cos,cos求导-sin
			v.Position.y = radius*cosf(phi);
			v.Position.z = radius*sinf(phi)*sinf(theta);

			// Partial derivative of P with respect to theta//求关于theta的偏导数(只对theta求导)
			...
			v.TexC.x = theta / XM_2PI;//theta向量投影在xz平面投影向量与x中夹角，范围0到2π
			v.TexC.y = phi / XM_PI;//phi为为向量离y轴角度，范围为0-π
	...
	//
	//按理说i从0开始，但从0开始电视（0,1,0）是条线，略过可节省计算（stackCount比较大的时候）
	...
	UINT baseIndex = 1;//top的那个顶尖
	UINT ringVertexCount = sliceCount+1;
	for(UINT i = 0; i < stackCount-2; ++i)//最后一排的线向下组成的三角形实际是末端点的三角形
	{
		for(UINT j = 0; j < sliceCount; ++j)
		{
